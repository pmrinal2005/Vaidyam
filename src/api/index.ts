...
import type { Bindings, Envelope, Provenance } from '../lib/types'
import {
  geoFromRequest,
  fetchAir,
  fetchWeather,
...
import { buildVitals, buildMedications, buildGraph, personalizedPageRank, seedsFromQuery, riskScores, pearson, type Vitals } from '../lib/twin'
import { routeQuery, runSwarm, buildCascade, providers, AGENT_DEFS } from '../lib/inference'
import { CLAIM_DEFS, buildAttestation, dpAggregate, quantizationStats } from '../lib/privacy'
import { simulate, buildLevers, literatureTerms } from '../lib/counterfactual'
import { seeded, clamp, round, gauss, sha256 } from '../lib/rand'
...
api.post('/swarm', async (c) => {
  const started = Date.now()
  const ctx = await twinContext(c)
  const body = await c.req.json<{ query?: string; live?: boolean }>().catch(() => ({}))
...
api.post('/counterfactual', async (c) => {
  ...
  const ctx = await twinContext(c)
  const prov = ctx.prov.slice()
  const body = await c.req.json<{ interventions?: Record<string, number>; horizonMonths?: number; withLiterature?: boolean }>().catch(() => ({}))
...
api.post('/zk/prove', async (c) => {
  const started = Date.now()
  const ctx = await twinContext(c)
  const body = await c.req.json<{ claim?: string; windowDays?: number }>().catch(() => ({}))
